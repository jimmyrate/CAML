...
BASIC_INFORMATION = {
    "bigscience/T0_3B": {
        "base_model": "checkpoints/T0_3B",
        "dump_dir": f"",
        "load_dir": f"checkpoints/T0_3B-finetuned",
    },
    "t5-base": {
        "base_model": "checkpoints/t5-base",
        "dump_dir": f"",
        "load_dir": f"checkpoints/t5-base-finetuned",
    },
    "t5-large": {
        "base_model": "t5-large",
        "dump_dir": f"",
        "load_dir": f"checkpoints/t5-large-finetuned",
    },
}
...
def topk_values_mask_single(M, K=0.7):
    # 如果 K 大于 1，则认为输入为百分比，例如 70 表示 70%
    if K > 1:
        K /= 100
    ...
    n, d = M.shape
    # 计算需要保留的元素个数（保留 top k 个数值较大的元素）
    k = int(d * K)
    k = d - k  # 转换为保留前 k 个元素

    # 对每一行计算按绝对值排序的第 k 个最小的元素
    kth_values, _ = M.abs().kthvalue(k, dim=1, keepdim=True)
    # 构造掩码：保留绝对值大于等于第 k 个最小值的元素
    mask = M.abs() >= kth_values
    # 对于原本是一维的情况，恢复为一维输出
    M_trimmed = M * mask
    if orig_shape == M.squeeze().shape:
        return M_trimmed.squeeze()
    return M_trimmed
...
def merge_models(selected_tv, flat_pretrain, lam, pcb_ratio, reference_state_dict, remove_keys):
    """
    输入:
      selected_tv: 一个形状为 (m, dim) 的张量，m 表示参与融合的子模型数目
      flat_pretrain: 预训练模型对应的向量
      lam: 融合的缩放系数
      pcb_ratio: PCB_merge 中使用的比例参数
      reference_state_dict: 用于转换向量到 state_dict 的参考模型（比如第一个子模型）
      remove_keys: 在向量化和还原过程中需要忽略的键
    过程:
      1. 调用 PCB_merge 对输入任务向量进行剪裁，返回 clamped_tv（依然是形状 (m, dim)）
      2. 对剪裁后的任务向量求平均，得到融合后的任务向量 merged_tv
      3. 计算融合后模型的向量：flat_pretrain + lam * merged_tv
      4. 转换为 state_dict 返回
    """
    # 对选择的任务向量进行 PCB 剪裁，得到剪裁后的版本
    clamped_tv, clamped_all_checks, scale = PCB_merge(selected_tv, pcb_ratio=pcb_ratio)
    # 对剪裁后的多个任务向量取平均
    merged_tv = clamped_tv.mean(dim=0)
    # 计算融合后的模型向量
    merged_check = flat_pretrain + lam * merged_tv
    # 转换回 state_dict 格式
...
def ties_trim(
    ...
    trim_checks, *_ = topk_values_mask_single(
        model_task_checks, K=reset_thresh
    )
